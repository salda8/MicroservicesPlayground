...
namespace StsServerIdentity.Services
{
    public class ProfileService : IProfileService
    {
        ...
        {
            Guard.Argument(context.Subject, nameof(context.Subject)).NotNull();

            ClaimsPrincipal subject = context.Subject;

            string subjectId = subject.Claims.FirstOrDefault(x => x.Type == JwtClaimTypes.Subject).Value;

            MongoIdentityUser user = await userManager.FindByIdAsync(subjectId);
